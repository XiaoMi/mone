...
import run.mone.m78.ip.bo.MethodInfo;
import run.mone.m78.ip.bo.ModuleInfo;
import run.mone.m78.ip.bo.PsiInfo;
import run.mone.m78.ip.service.ClassFinder;
    ...
    protected void doOKAction() {
        ...
        if (selectedTab.equals("Class")) {
            if (req.getCmd().equals("test") || req.getCmd().equals("docean_controller")) {
                String clazz = this.listMap.get("Class").getSelectedValue().getName();
                this.result.getData().put("class", clazz);
                PsiClass psiClass = CodeService.getPsiClass(project, context.getModule(), clazz);
