...
import run.mone.m78.ip.ui.ConfigUi;
...
public class TeslaConfigurable implements SearchableConfigurable {
    ...
    public void apply() {
        ...
        config.setAiProxy(configUi.getAiProxyTextField().getText().trim());
        if (configUi.getModelComboBox().getModel().getSize() > 0 && null != configUi.getModelComboBox().getSelectedItem()) {
            config.setModel(configUi.getModelComboBox().getSelectedItem().toString());
            AthenaContext.ins().setGptModel(config.getModel());
        }
        ComboBoxModel model = configUi.getModelComboBox().getModel();
        List<String> list = new ArrayList<>();
        for (int i = 0; i < model.getSize(); i++) {
            list.add(model.getElementAt(i).toString());
        }
        config.setModelList(list);
        PropertiesComponent.getInstance().setValue("", new Gson().toJson(config));
