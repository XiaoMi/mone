...
<template>
  <d2-container>
    ...
    <d2-module>
          ...
          <el-table-column label="操作" width="170" fixed="right">
            <template slot-scope="scope">
              <router-link
                ...
                :to="{ path: `/wiki/doc/detail/1/${filterPre}${scope.row.id}`}">
                文档
              </router-link>
              <el-button
                type="text"
     ...
     <el-dialog :title="`${title}通用filter`" :visible.sync="dialogVisible" width="800px">
        <el-form :model="formAdd" label-width="110px" ref="uploadForm" :rules="rules" size="mini">
          <el-form-item label="git地址:">
            ...
            <div style="display:inline-block; width:25%" >
              <el-form-item prop="group">
                  <el-input v-model="formAdd.group" placeholder="请输入组名" :disabled="this.title === '更新' ? true :false"></el-input>
              </el-form-item>
            </div>
            <el-input value="/" placeholder="" disabled style="width:6%"></el-input>
            <div style="display:inline-block; width:25%">
              <el-form-item prop="name">
                  <el-input v-model="formAdd.name" placeholder="请输入项目名" :disabled="this.title === '更新' ? true :false"></el-input>
              </el-form-item>
            </div>
          </el-form-item>
          ...
          <el-form-item label="commit" prop="commitId">
            ...
            <span v-if="commitLoading" @click="getCommitList"><i class="el-icon-refresh" style="font-size:19px; margin-left: 6px"></i></span>
            <span v-else><i class="el-icon-loading" style="font-size:19px; margin-left:6px;"></i></span>
          </el-form-item>
       </el-form>
  ...
  methods: {
      ...
      service({ url }).then(res => {
          ...
          this.pluginList = res.pluginList.map(item => {
            ...
            const totalRate = userRateBos.reduce((prev, cur) => {
              if (cur.accountId == this.info.uuid) rate = cur.rate
    ...
    getCommitList () {
      ...
      if(this.formAdd.group && this.formAdd.name){
        service({
          url: `/filter/commits?group=${this.formAdd.group}&name=${this.formAdd.name}`,
          method: 'GET'
        }).then( res => {
          res.forEach( item => {
