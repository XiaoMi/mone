  ...
  <div>
     ...
     <el-card style="margin-bottom:20px">
      <el-form :model="form" :rules="rulesTop" size="mini" label-width="110px" ref='formTop'>
       ...
       <el-form-item label="内存(M)" prop="memory">
          <el-input-number v-model="form.memory" controls-position="right" placeholder="内存大小" style="width: 15%" :min="2048" :max="(form.cpu || 1) * 4096"></el-input-number>
       </el-form-item>
    ...
    <el-card>
      <el-form :model="form" :rules="rulesBottom" size="mini" label-width="110px" ref='formBottom'>
       ...
       <el-form-item label="自定义环境变量" prop="dockerParamsStr">
          <el-input
            type="textarea"
            v-model="form.dockerParamsStr"
      ...
      this.$refs['formBottom'].validate(valid => {
        ...
        service({
          ...
          data: {
            ...form,
            envId: this.envId,
            dockerParams: JSON.parse(form.dockerParamsStr)
          }
        }).then(res => {
          if (Array.isArray(res)) {
            this.errorTitle = '升级配置失败'
    ...
    getDockerSetting () {
      ...
      service({
        ...
        if (res) {
          ...
          this.form = {
            ...res,
            dockerParamsStr: JSON.stringify(res.dockerParams || {})
          }
        }
      })
    },
