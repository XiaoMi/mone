      ...
      <div class="input side-box">
        ...
        <el-form ref="form" :rules="rules" :model="input" size="mini" label-width="100px" style="width:550px;">
            <el-form-item
             label="服务名称"
             prop="serviceName">
              <el-input placeholder="eg. com.xiaomi.youpin.metis.service.MetisDubboService" width="100px" size="mini" @change="handleChangeServiceName" v-model.trim="input.serviceName"></el-input>
            </el-form-item>
              <el-form-item
               prop="version"
               label="服务版本">
              ...
              <el-form-item
               ...
               label="参数类型">
              <el-input
               placeholder='eg. ["java.lang.String", "int", "com.xiaomi.youpin.zzytest.api.service.MyParam"]'
...
export default {
  ...
  created () {
    ...
    //       "username":"cat",
    //       "token":"1111",
    //       "urls":["/mtop/arch/test/getAccountsByUrls"]
    //     }
  ...
  methods: {
    ...
    handleChangeServiceName () {
      let evt = this.input.serviceName
      let version = this.input.version
      if (evt !== null || evt.trim() !== '') {
        let searchWord = (this.input.group && this.input.group.length > 0)
          ? `${evt}:${version}:${this.input.group}`
          : `${evt}:${version}`
        this.getInstance(searchWord)
      }
    },
