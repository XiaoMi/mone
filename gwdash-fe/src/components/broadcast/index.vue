      ...
      <div class="broadcast_box" v-show="isOpen" ref='broadcastBox'>
        ...
        <div class="message-box" ref="msgBox">
          <template v-for="(messageObj,index) in messageList">
              <MiddleBar v-if="messageObj.operation==='open'|| messageObj.operation==='connected'||messageObj.operation==='disconnected'" :message="messageObj"></MiddleBar>
              <LeftBar v-else-if="messageObj.operation==='receive'" :message="messageObj"></LeftBar>
              <RightBar v-else-if="messageObj.operation==='send'" :message="messageObj"></RightBar>
          </template>
        </div>
        <div class="send-message-box">
          <el-input v-model="broadcastInput" @enter placeholder="ctrl+enter 发送" size="mini"></el-input>
...
export default {
  ...
  methods:{
    ...
    data2Message(msg){
      ...
      if(serverEnv==='local'){
        this.userInfo.username="zhangxiuhua"
        this.userInfo.name="张秀华"
      }
      let msgBase={
        operation:'send',
