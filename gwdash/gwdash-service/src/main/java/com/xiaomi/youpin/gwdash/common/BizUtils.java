...
public final class BizUtils {
    ...
    public static CheckResult chkNewAccountParam(AccountParam param) {
        ...
        if (null == param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的账号参数");
        ...
        if (StringUtils.isBlank(param.getUsername()) ||
            ...
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的账号参数");
        }

        if (null == param.getRole() || !Consts.ValidRoles.contains(param.getRole())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的账号角色参数");
        }

        return new CheckResult(true, CommonError.Success.code, "");
    }
    ...
    public static CheckResult chkUpdateAccountParam(AccountParam param) {
        ...
        if (null == param.getId() || param.getId() <= 0) {
            return new CheckResult(false,
                    CommonError.InvalidIDParamError.code, "无效的账号ID参数");
        }
    ...
    public static CheckResult chkNewApiGroupInfoParam(ApiGroupInfoParam param) {
        if (null ==  param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的ApiGroupInfo参数");
        }

        if (StringUtils.isBlank(param.getName())) {
            return new CheckResult(false,
                    CommonError.InvalidApiGroupNameError.code, "无效的ApiGroup名称");
        }

        if (StringUtils.isBlank(param.getDescription())) {
            return new CheckResult(false,
                    CommonError.InvalidApiGroupDescError.code, "无效的ApiGroup描述信息");
        }

        if (StringUtils.isBlank(param.getBaseUrl())) {
            return new CheckResult(false,
                    CommonError.InvalidApiGroupBaseUrlError.code, "无效的ApiGroup BaseUrl信息");
        }
    ...
    public static CheckResult chkNewApiInfoParam(ApiInfoParam param) {
        if (null ==  param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的ApiInfo参数");
        }

        if (StringUtils.isBlank(param.getName())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的Api名称");
        }
// 2019-09-16 17:16:19 黄江需求 描述字段改为可选
//        if (StringUtils.isBlank(param.getDescription())) {
//            return new CheckResult(false,
//                    CommonError.InvalidParamError.code, "无效的Api描述");
//        }

        if (!RouteType.isValidRouteType(param.getRouteType())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的route类型");
        }

        if (StringUtils.isBlank(param.getUrl())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的路径");
        }
    ...
    public static CheckResult chkNewHttpApiParam(ApiInfoParam param) {
        if (StringUtils.isBlank(param.getPath())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的目标路径信息");
        }

        String url = param.getPath().toLowerCase();
        if (!url.startsWith("http://") && !url.startsWith("https://")) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的目标路径格式");
        }
    ...
    public static CheckResult chkNewDubboApiParam(ApiInfoParam param) {
        if (StringUtils.isBlank(param.getServiceName())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的服务名称信息");
        }

        if (StringUtils.isBlank(param.getMethodName())) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的方法名称信息");
        }
    ...
    public static CheckResult chkAgentUpdateParam(AgentUpdateParam param) {
        if (null == param) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "空的Agent更新参数");
        }
        if (param.getId() == null) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的Agent更新参数");
        }
    ...
    public static CheckResult chkUpdateApiGroupParam(ApiGroupInfoUpdateParam param) {
        if (null ==  param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的ApiGroupInfo更新参数");
        }

        if (param.getId() == null || param.getId() <= 0) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的ApiGroupInfo id参数");
        }

        if (StringUtils.isBlank(param.getBaseUrl())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "API前缀不可为空");
        }
    ...
    public static CheckResult chkUpdateApiInfoParam(ApiInfoUpdateParam param) {
        if (null ==  param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的ApiInfo更新参数");
        }

        if (param.getId() == null || param.getId() <= 0) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的ApiInfo id参数");
        }
    ...
    public static CheckResult chkGitlabOptParam(GitlabOptParam param) {
        if (param == null) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的参数信息");
        }

        if (StringUtils.isBlank(param.getProjectId())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的projectId");
        }

        if (StringUtils.isBlank(param.getToken())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的token");
        }

        if (StringUtils.isBlank(param.getPath())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的path");
        }

        if (StringUtils.isBlank(param.getBranch())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的branch");
        }
        return new CheckResult(true, CommonError.Success.code, "");
    }
    ...
    public static CheckResult chkApiDebugParam(ApiDebugParam param) {
        if (null ==  param) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "空的API Debug参数");
        }

        if (param.getAid() == null || param.getAid() <= 0) {
            return new CheckResult(false,
                    CommonError.InvalidParamError.code, "无效的API id参数");
        }

        if (StringUtils.isBlank(param.getHttpMethod()) && !Consts.ValidHttpMethods.contains(param.getHttpMethod())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的http method");
        }

        if (StringUtils.isBlank(param.getUrl())) {
            return new CheckResult(false, CommonError.InvalidParamError.code, "无效的URL");
        }
