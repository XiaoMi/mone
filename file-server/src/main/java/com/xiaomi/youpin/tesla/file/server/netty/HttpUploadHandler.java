    ...
    protected void channelRead0(ChannelHandlerContext ctx, HttpObject httpObject) throws Exception {
        try {
            if (httpObject instanceof HttpRequest) {
                ...
                if ((request.uri().startsWith(Cons.UPLOAD) || requestUri.getPath().startsWith(Cons.UPLOAD) ) && request.method().equals(HttpMethod.POST)) {
                    len = Long.parseLong(request.headers().get(HttpHeaderNames.CONTENT_LENGTH, "0"));
                    log.info("len:{}", len);
