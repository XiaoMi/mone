<!--
 * @Description:
 * @Date: 2024-03-11 15:01:26
 * @LastEditTime: 2024-09-21 18:24:50
-->
<template>
      ...
      <li v-for="(item, index) in props.data" :key="index">
        ...
        <div class="foot">
          ...
          <div class="foot-bottom">
            ...
            <div class="link">
              <el-button type="primary" plain size="large" @click="visit(item)"
    ...
    <ul v-else>
      ...
      >
        <div class="probot-top">
          ...
          <div class="user"> <el-tag type="primary" size="small" v-if="item.official">官方插件</el-tag> @{{ item.userName }}</div>
          <div class="describe">
        ...
        <div class="foot">
          <div class="tag" v-if="item.pluginCategory">
            <el-popover placement="top" title="" popper-class="tag-popover" trigger="hover">
              <template #reference>
                <span>
                  <el-link
                    type="primary"
                    v-for="(v, i) in item?.pluginCategory"
                    :key="i"
                    :underline="false"
                    >{{ v ? '# ' + v : '' }}</el-link
                  >
              ...
              <div v-for="(v, i) in item.pluginCategory" :key="i" class="tag-item">
                {{ v }}
              </div>
            </el-popover>
          </div>
          <div class="foot-bottom">
            <div class="yinyong">
              <span class="num">{{ item.botRefCnt || 0 }} Bots 使用</span>
            </div>
          </div>
        </div>
      </li>
...
.loading-container {
  ...
  .spinnerSexBox {
    background: var(--oz-menu-active-color);
    border-radius: 50%;
    height: 1em;
    margin: 0.1em;
    width: 1em;
