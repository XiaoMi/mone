...
export function stox(wb: XLSX.WorkBook) {
  const out: { name: any; rows: {} }[] = []
  // console.log('wb', wb)
  wb.SheetNames.forEach(function (name: string | number) {
    ...
    aoa.forEach(function (r, i) {
      const cells = {}
      r.forEach(function (c: any, j: string | number) {
    ...
    o.merges = []
    ;(ws['!merges'] || []).forEach(function (merge: XLSX.Range, i: string | number) {
 ...
 */
export function xtos(sdata: { rows: any; name: string | undefined }[]) {
  const out = XLSX.utils.book_new()
  sdata.forEach(function (xws: { rows: any; name: string | undefined }) {
    const ws = {}
