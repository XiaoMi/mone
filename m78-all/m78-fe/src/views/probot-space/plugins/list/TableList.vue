<template>
  <el-table :data="props.tableList" style="width: 100%" stripe>
    ...
    <el-table-column prop="meta" label="输入参数" min-width="340">
      <template #default="scoped">
        <el-tag
          type="info"
          size="small"
          v-for="(item, index) in JSON.parse(scoped.row?.meta)?.input" :key="index">
          {{ item.name }}</el-tag
        >
      </template>
    </el-table-column>
  ...
  <DebugDawer v-model="state.debug.visible" :data="{ id: state.debug.id }" @onOk="emits('onOk')" />
  <EditComp
