<template>
      ...
      >
        <!-- <div>
          <el-form-item prop="desc" label="" label-width="10px">
            <div class="card">
              <div class="card-header"><el-checkbox v-model="ruleForm.m78"></el-checkbox></div>
              <div class="card-body">
                <h3 class="name"><span>m78</span></h3>
                <div class="desc"><span>m78描述</span></div>
              </div>
            </div>
          </el-form-item>
        </div> -->
  ...
  ></DeployOpenIdDialog>
  <!-- 飞书-配置openId -->
  <DeployLetterFeishuDialog
    v-model="letterOpenIdVisible"
    :data="{ openId: letterOpenId, secret: secret }"
    @submit="letterOpenidChange"
  ></DeployLetterFeishuDialog>
</template>
...
import DeployLetterFeishuDialog from './DeployLetterFeishuDialog.vue'
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      if (ruleForm.feishu && !relationFlag.value &&(!secret.value || !letterOpenId.value)) {
        ElMessage.error('若您的发布平台选择飞书则必须配置飞书机器人！')
        return
      }
      const items: number[] = []
      // if (m78) item.push('m78')
      if (ruleForm.feishu) items.push(1)
      if (ruleForm.weixin) items.push(2)
      if (ruleForm.weixin && !openId.value) {
        ElMessage.error('选择微信请配置微信openId')
        return false
      }
      publishBot({
        ...
        openId: JSON.stringify({
          ...
          secret: ruleForm.feishu ? secret.value : '',
...
const secret = ref('')
const letterOpenidChange = (obj) => {
  ruleForm.feishu = true
  letterOpenId.value = obj.openId
  secret.value = obj.secret
}
const optimize = () => {
...
const handleOpen = () => {
  const { publishRecordDTOS = [], botCategory = [], botInfo = {},imRelationDTOS=[] } = props.data
  ...
  ruleForm.categoryIds = probotStore.categoryList['']
    ...
    ?.map((item) => item.id)
    letterOpenId.value=imRelationDTOS[0]?.relationFlag;
    relationFlag.value=imRelationDTOS[0]?.relationFlag?true:false
