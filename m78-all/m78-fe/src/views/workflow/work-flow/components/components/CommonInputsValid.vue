    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item name="1">
        <template #title>
          <div class="t-box">
            <div class="t-left">
              <CollapseTitle :activeNames="activeNames" title="输入"
                :content="titleDes || '输入需要添加到提示词的信息，这些信息可以被下方的提示词引用'" tipClass="title-tooltip" :showAdd="false" />
            </div>
        ...
        <div v-for="(item, i) in inputs" :key="item" class="input-item">
          <el-form-item :label="i < 1 ? '变量名' : ''" :prop="'inputs.' + i + '.name'" :rules="{
            ...
            <el-input v-model="item.name" placeholder="请输入参数名" style="width: 200px" maxlength="20"
              v-if="!nameDisabled" />
            <el-tooltip v-else effect="dark" :content="showName[item.name] || item.name" placement="top">
              <span class="name-span" :class="showValueType ? 'show-value-type' : 'hide-value-type'">{{
                showName[item.name] ||
                item.name }}</span>
            </el-tooltip>
          ...
          <div class="val-box">
            <el-form-item :label="i < 1 ? '变量值' : ''" :prop="'inputs.' + i + '.type'" :rules="{
            ...
            <el-form-item label="" :class="i == 0 ? 'empty-item' : ''" :prop="'inputs.' + i + '.value'" :rules="{
              validator: (rule, value, cb) => {
                validateFn(rule, value, cb, inputs[i])
              },
              trigger: 'blur'
            }" v-if="item.type == 'value'">
              <el-input v-model="item.value" placeholder="请输入参数值" :style="refreStyle" />
            </el-form-item>
            <el-form-item v-else label="" :class="i == 0 ? 'empty-item' : ''" :prop="'inputs.' + i + '.referenceInfo'"
              :rules="{
                validator: (rule, value, cb) => {
                  validateFn(rule, value, cb, inputs[i])
                },
            ...
            <el-form-item :class="i == 0 ? 'empty-item' : ''" v-if="showDelete">
...
const validateFn = (rule, value, cb, inputItem) => {
  if (!inputItem.required) return cb()
  return validateRef(rule, value, cb, inputItem)
}
</script>
