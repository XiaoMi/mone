  ...
  <el-dialog v-model="show" title="表绑定" width="1120" :before-close="beforeClose">
    <template #header>
      <div class="my-header">
        <h2>表绑定</h2>
        <GenByAi
          :workspaceId="formData.botInfo.workspaceId"
          @genFinish="genFinish"
          ref="GenByAiRef"
        />
      </div>
    ...
    <el-form ref="formRef" :model="form" label-position="top">
      ...
      <el-form-item prop="desc" label="数据表描述">
        <el-input v-model="form.desc" :autosize="{ minRows: 3, maxRows: 5 }" type="textarea" />
      </el-form-item>
      <el-form-item prop="demo" label="示例">
        <el-input v-model="form.demo" :autosize="{ minRows: 3, maxRows: 5 }" type="textarea" />
      </el-form-item>
      <ul class="list-ul">
        <li v-for="(item, i) in form.list" :key="i" class="list-li">
          <el-form-item
            :prop="'list.' + i + '.name'"
            :rules="{
              required: true,
...
import { initFormData, updateFormInfo } from '@/api/probot.ts'
import DataTypeSel from '@/views/probot/components/DataTypeSel'
import { Delete } from '@element-plus/icons-vue'
import { useRoute } from 'vue-router'
import LLMModelSel from '@/components/LLMModelSel.vue'
import { ElMessage } from 'element-plus'
import GenByAi from '../components/GenByAi'
...
const reqFn = () => {
  initFormData({ botId: route.params.id }).then(({ code, data, message }) => {
    if (code === 0 && data) {
      form.value.tableName = data.tableName
      form.value.desc = data.tableDesc
      form.value.demo = data.demo
      form.value.list = (data.columnInfoList || []).map((it) => {
        return {
          ...it
        }
      })
      emits('update', [{ ...data, name: data.tableName }])
    } else {
...
const confirm = async () => {
  ...
  loading.value = true
  updateFormInfo({
    columnInfoList: form.value.list,
    tableName: form.value.tableName,
    botId: route.params.id,
    demo: form.value.demo,
    workspaceId: props.formData.botInfo.workspaceId
  })
    .then(
      ({ code, data, message }) => {
