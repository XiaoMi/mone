 ...
 * @LastEditTime: 2024-03-26 17:12:11
-->
<template>
...
const generateAvatar = async () => {
  ...
  try {
    ...
    let response = {} as any
    if (props.type === 'bot') {
      response = await createBotAvatar({
        botName: botName.value,
        botDesc: props.remark
      })
    } else {
      response = await generateImage({
        name: props.formData.pluginOrgName,
        desc: props.remark
      })
    }
    if (response.code === 0) {
