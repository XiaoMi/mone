 ...
 * @LastEditTime: 2024-03-28 18:08:50
-->
<template>
  <el-dialog
...
import { create, update } from '@/api/probot-knowledge'
...
const rules = reactive<FormRules<RuleForm>>({
  ...
  avatarUrl: [{ required: true, message: '请选择知知识库图标', trigger: 'change' }],
...
const sure = () => {
  submitForm(formRef.value, form).then(() => {
    loading.value = true
    let request = props.data.id ? update : create
    let labels = ''
