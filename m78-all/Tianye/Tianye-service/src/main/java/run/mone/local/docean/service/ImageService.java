...
public class ImageService {
    ...
    public String imageUnderstand(VisionReq req, int timeout){
        try {
            String aiProxy = ((Config) Ioc.ins().getBean(Config.class)).get("ai.proxy", "");
            Map<String, String> header = new HashMap<>();
            header.put("Content-Type", "application/json");
            String res = HttpClientV5.post(aiProxy + "/api/z-proxy/multiModal/image", GsonUtils.gson.toJson(req), header, timeout);
