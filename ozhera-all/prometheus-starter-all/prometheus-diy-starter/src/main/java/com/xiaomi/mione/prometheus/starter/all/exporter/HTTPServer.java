...
public class HTTPServer {
     ...
     */
    public HTTPServer(HttpServer httpServer, Map<String, CollectorRegistry> registryMap, boolean daemon) throws IOException {
    ...
    public HTTPServer(HttpServer httpServer, final CollectorRegistry registry, boolean daemon) throws IOException {
        if (httpServer.getAddress() == null) {
            throw new IllegalArgumentException("HttpServer hasn't been bound to an address");
        }
        server = httpServer;
        HttpHandler mHandler = new HTTPMetricHandler(new HashMap<String, CollectorRegistry>(){{
            put("default",registry);
        }});
        server.createContext("/", mHandler);
        server.createContext("/metrics", mHandler);
        server.createContext("/-/healthy", mHandler);
     ...
     */
    public HTTPServer(InetSocketAddress addr, final CollectorRegistry registry, boolean daemon) throws IOException {
        this(HttpServer.create(addr, 3), new HashMap<String, CollectorRegistry>() {{
            put("default", registry);
        }}, daemon);
    }
