    ...
    if (B3Propagator.SINGLE_HEADER_DEBUG.equals(sampled)) {
      return Optional.of(
          context
              ...
              .with(Span.wrap(Common.buildSpanContext(traceId, spanId, Common.TRUE_INT, heraContext))));
    }

    return Optional.of(context.with(Span.wrap(Common.buildSpanContext(traceId, spanId, sampled, heraContext))));
  }
}
