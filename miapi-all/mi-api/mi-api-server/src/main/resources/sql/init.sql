...
) ENGINE=InnoDB AUTO_INCREMENT=197 DEFAULT CHARSET=utf8 COMMENT='索引组映射表';
...
) ENGINE=InnoDB AUTO_INCREMENT=4210 DEFAULT CHARSET=utf8 COMMENT='请求示例';
...
) ENGINE=InnoDB AUTO_INCREMENT=2135 DEFAULT CHARSET=utf8 COMMENT='返回示例';
...
CREATE TABLE `api_test_case` (
  ...
  `http_headers` varchar(255) NOT NULL DEFAULT '' COMMENT 'http请求头json',
  ...
  `x5_app_id` int(11) NOT NULL DEFAULT '0' COMMENT 'x5的appid',
...
CREATE TABLE `eo_api` (
  ...
  UNIQUE KEY `uk_api` (`apiURI`,`apiRequestType`) USING BTREE,
  KEY `apiID` (`apiID`),
  KEY `projectID` (`projectID`)
) ENGINE=InnoDB AUTO_INCREMENT=3107 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='api总表';
