...
import ReturnExmp from './ReturnExmp'
...
export default {
  ...
  watch: {
    ...
    apiDetail: {
      handler (val) {
        ...
        if (val.apiErrorCodes && val.apiErrorCodes.length) {
          this.apiErrorCodes = val.apiErrorCodes
        } else {
          this.apiErrorCodes = []
        }
      },
  ...
  methods: {
    handleChangeCode (val) {
      this.$store.dispatch('apilist.add/changeHttpParam', { apiErrorCodes: val })
    }
  }
}
</script>
