      ...
      >
        ...
        <template #header>
          <span class="common-table-title">{{$i18n.t('table.parameterValue')}}</span>
				...
				<template #default="scope">
					<el-input :disabled="!disabledChildrenTree(scope) || scope.row.isRoot" :placeholder="$i18n.t('placeholder.pleaseEnter')" v-model="scope.row.paramValue" autocomplete="off"></el-input>
				</template>
			</el-table-column>
			<el-table-column
				prop="paramNote"
  ...
  methods: {
    ...
    init (initData) {
      ...
      if (initData && initData.length) {
        ...
        if (tableData[0].childList.every(item => !!item.paramType) && (tableData[0].paramType !== DATA_TYPE_KEY.array)) {
  ...
  watch: {
    tableData: {
      handler: function (val, oldVal) {
        if (val[0].isRoot && val[0].paramType === DATA_TYPE_KEY.array && !val[0].childList.length) {
