...
export default {
  ...
  watch: {
    ...
    tableData: {
      handler: function (val, oldVal) {
        ...
        this.$emit("onChange", { apiRequestParam: val })
      },
      deep: true
    },
    formJsonData: {
      handler (newVal) {
        this.$emit("onChange", { apiRequestParam: newVal })
      },
      deep: true
    },
    type: {
      handler (newVal, old) {
        if (newVal !== old) {
          // this.$emit("onChange", { apiRequestParamType: newVal })
        }
        switch (newVal) {
          case API_REQUEST_PARAM_TYPE.FORM_DATA:
            this.$emit("onChange", { apiRequestParam: this.tableData })
            break
          case API_REQUEST_PARAM_TYPE.JSON:
            this.$emit("onChange", { apiRequestParam: this.formJsonData })
            break
  ...
  methods: {
    ...
    handleInputJson (content) {
      this.$emit("onChange", { apiRequestRaw: JSON.stringify(content) })
    },
    handleError () {
      this.$emit("onChange", { apiRequestRaw: JSON.stringify({}) })
    },
    handleChildList (arr) {
