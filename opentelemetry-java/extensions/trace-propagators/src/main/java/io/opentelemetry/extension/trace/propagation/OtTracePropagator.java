...
public final class OtTracePropagator implements TextMapPropagator {
  ...
  public <C> Context extract(Context context, @Nullable C carrier, TextMapGetter<C> getter) {
    ...
    SpanContext spanContext = buildSpanContext(traceId, spanId, sampled, heraContext);
    if (!spanContext.isValid()) {
      return context;
    }
  ...
  static SpanContext buildSpanContext(String traceId, String spanId, String sampled, String heraContext) {
    if (!Common.isTraceIdValid(traceId) || !Common.isSpanIdValid(spanId)) {
      return SpanContext.getInvalid();
    }
    return Common.buildSpanContext(traceId, spanId, sampled, heraContext);
  }
}
