    ...
    private String chaosToolSchema = """
            {
                ...
                "properties": {
                    ...
                    "logIp": {
                        "type": "string",
                        "description": "机器IP或容器IP，用于筛选特定机器的日志"
                    },
                    "page": {
                        "type": "integer",
    ...
    public Flux<McpSchema.CallToolResult> apply(Map<String, Object> args) {
        return Flux.defer(() -> {
            try {
                ...
                // 获取可选参数：logIp
                String logIp = getStringParam(args, "logIp");
                if (logIp.isEmpty()) {
                    logIp = null;
                }
                ...
                log.info("开始查询日志，level: {}, projectId: {}, pipelineId: {}, startTime: {}, endTime: {}, traceId: {}, logIp: {}, page: {}, pageSize: {}",
                        level, projectId, pipelineId, startTime, endTime, traceId, logIp, page, pageSize);

                // 调用服务查询日志
                String result = logQueryService.queryLogs(level, projectId, pipelineId, startTime, endTime, traceId, logIp, page, pageSize);
