  ...
  <el-popover placement="right" :width="200" trigger="click">
    ...
    <div class="flow-data">
      ...
      <el-collapse v-model="activeNames" size="small">
        <el-collapse-item v-for="item in allNodes" :key="item.nodeId" :name="item.nodeId" :title="item.nodeName"
          class="flow-data-item" :class="`status-${item.outputs?.status}`">
          <template #title>
            {{ item.nodeName }}
            <TimerTag class="timer" :type="flowStatusArr.find((it) => it.code === item.outputs?.status)?.tagType"
              v-if="[2, 3].includes(item.outputs?.status)">{{ formatTime(item.outputs?.durationTime) }}</TimerTag>
          </template>
          <template v-if="item.inputs?.length > 0">
            <p class="title title-input">INPUT</p>
            <JsonViewer :value="item.inputs || []" copyable boxed sort theme="dark" :expand-depth="1" :key="item.key" />
          </template>
          <template v-if="item.outputs?.length > 0">
            <div>{{ item.outputs }}</div>
            <p class="title ouput-t">OUTPUT</p>
            <JsonViewer :value="item.outputs || []" copyable boxed sort theme="dark" :expand-depth="1"
              :key="item.key" />
          </template>
        </el-collapse-item>
      </el-collapse>
...
import { flowHeaderStatus, formatTime, flowStatusArr } from './baseInfo'
