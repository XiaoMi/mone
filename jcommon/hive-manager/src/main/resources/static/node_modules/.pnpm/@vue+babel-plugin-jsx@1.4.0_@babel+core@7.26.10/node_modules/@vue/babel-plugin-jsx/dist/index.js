...
var checkIsComponent = (path, state) => {
  ...
  return !((_b = (_a = state.opts).isCustomElement) == null ? void 0 : _b.call(_a, tag)) && shouldTransformedToSlots(tag) && !(0, import_shared.isHTMLTag)(tag) && !(0, import_shared.isSVGTag)(tag);
};
...
var getTag = (path, state) => {
  ...
  if (namePath.isJSXIdentifier()) {
    ...
    if (!(0, import_shared.isHTMLTag)(name) && !(0, import_shared.isSVGTag)(name)) {
...
var isConstant = (node) => {
  ...
  if (t.isTemplateLiteral(node) ? !node.expressions.length : t.isLiteral(node)) {
    return true;
  }
  return false;
};
...
var resolveDirective = (path, state, tag, directiveName) => {
  ...
  if (path.scope.references[referenceName]) {
    return t2.identifier(referenceName);
  }
...
var transformJSXElement = (path, state) => {
  ...
  if (directives.length && directives.some(
    (d) => {
      var _a2, _b2;
      return ((_b2 = (_a2 = d.elements) == null ? void 0 : _a2[0]) == null ? void 0 : _b2.type) === "CallExpression" && d.elements[0].callee.type === "Identifier" && d.elements[0].callee.name === "_resolveDirective";
    }
  )) {
    let currentPath = path;
    while ((_a = currentPath.parentPath) == null ? void 0 : _a.isJSXElement()) {
      currentPath = currentPath.parentPath;
      currentPath.setData("slotFlag", 0);
    }
  }
  const slotFlag = (_b = path.getData("slotFlag")) != null ? _b : slotFlags_default.STABLE;
  ...
  if (children.length > 1 || slots) {
    VNodeChild = isComponent ? children.length ? t3.objectExpression(
      [
        ...
        optimizeSlots && t3.objectProperty(t3.identifier("_"), t3.numericLiteral(slotFlag))
      ].filter(Boolean)
    ...
    const objectExpression4 = t3.objectExpression(
      [
        ...
        optimizeSlots && t3.objectProperty(
          t3.identifier("_"),
...
var index_default = (0, import_helper_plugin_utils.declare)(
  (api, opt, dirname) => {
    const { types } = api;
    let resolveType;
    if (opt.resolveType) {
