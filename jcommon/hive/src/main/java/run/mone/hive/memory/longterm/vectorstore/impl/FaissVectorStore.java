...
@Slf4j
@Data
public class FaissVectorStore implements VectorStoreBase {
    private final VectorStoreConfig config;
    
    public FaissVectorStore(VectorStoreConfig config) {
        this.config = config;
    }
    
    @Override
    public void insert(List<List<Double>> vectors, List<String> ids, List<Map<String, Object>> payloads) {
        throw new UnsupportedOperationException("FAISS implementation coming soon");
    }
