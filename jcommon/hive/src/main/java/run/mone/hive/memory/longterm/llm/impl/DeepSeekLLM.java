...
public class DeepSeekLLM implements LLMBase {
    ...
    public DeepSeekLLM(LlmConfig config) {
        this.config = config;
        this.gson = new Gson();
        ...
        if (this.apiKey == null || this.apiKey.trim().isEmpty()) {
            throw new IllegalArgumentException("DeepSeek API key is required. Set it in config or DEEPSEEK_API_KEY environment variable.");
        }
    ...
    public String generateResponse(List<Map<String, Object>> messages, String responseFormat) {
        try {
            ...
            log.error("Error generating response from DeepSeek", e);
            throw new RuntimeException("Failed to generate response from DeepSeek", e);
        }
    }
    ...
    public String generateResponse(List<Map<String, Object>> messages, 
        ...
        try {
            ...
            log.error("Error generating response with tools from DeepSeek", e);
            throw new RuntimeException("Failed to generate response with tools from DeepSeek", e);
        }
    }
    ...
    public boolean supportsVision() {
        ...
        return model.contains("vision") || model.contains("vl");
    }
    ...
    private String sendRequest(Map<String, Object> requestBody) throws Exception {
        ...
        if (response.statusCode() != 200) {
            String errorMsg = String.format("DeepSeek API request failed with status %d: %s", 
    ...
    private String parseResponse(String responseJson) {
        try {
            ...
            if (jsonResponse.has("error")) {
                ...
                String errorMsg = String.format("DeepSeek API error: %s", error.get("message").getAsString());
                log.error(errorMsg);
                throw new RuntimeException(errorMsg);
            }
            ...
            log.error("Error parsing DeepSeek response: {}", responseJson, e);
