...
### scripts/
Executable Python code managed by uv. Run scripts with: `uv run python scripts/script_name.py`
...
**Dependencies:** Add required packages to `pyproject.toml` under `[project.dependencies]`
...
This script is managed by uv and should be run with:
...
def main():
    ...
    print("Running with uv-managed dependencies")
...
def init_skill(skill_name, path):
    ...
    python_version_path = skill_dir / '.python-version'
    try:
        python_version_path.write_text("3.11\n")
        print("✅ Created .python-version")
    except Exception as e:
        print(f"❌ Error creating .python-version: {e}")
        return None
    ...
    try:
        ...
        print(f"⚠️  Warning: uv sync failed: {e.stderr}")
        print("   You can run 'uv sync' manually later")
    except FileNotFoundError:
        print("⚠️  Warning: uv not found in PATH")
    ...
    print("4. Test scripts with: uv run python scripts/example.py")
