...
//        api.getApiClient().setDebugging(true);
    ...
    public V1Pod getPodOfIP(String ip) throws ApiException {
        V1PodList l = new CoreV1Api(client).
    ...
    public List<V1Pod> getPodsOfIP(String ip, String labelSelector) throws ApiException {
        V1PodList l = new CoreV1Api(client).
                listPodForAllNamespaces(null, null,
                        "status.podIP=" + ip,
                        labelSelector, null, null, null, 5, null);
        if (l.getItems().size() <= 0) {
            return null;
        }
        return l.getItems();
    ...
    public V1Node getNode(String name) throws ApiException {
        return new CoreV1Api(client).readNode(name, null, null, null);
    }

    public V1PodList getPodsOnNode(String node, String namespace, String labelSelector) throws ApiException {
        V1PodList l;
        if (namespace == null) {
            l = new CoreV1Api(client).listPodForAllNamespaces(null, null,
                    "spec.nodeName=" + node,
                    labelSelector, null, null, null, null, null);
        } else {
            l = new CoreV1Api(client).listNamespacedPod(namespace, null, null, null,
                    "spec.nodeName=" + node,
                    labelSelector,
                    null, null, 5, null);
        }
    ...
    public V1Pod getPodOnNode(String node, String namespace, String labelSelector) throws ApiException {
        V1PodList l = getPodsOnNode(node, namespace, labelSelector);
        if (l.getItems().size() <= 0) {
            return null;
        }
        return l.getItems().get(0);
