...
public class MybatisPlugin implements IPlugin {
    ...
    private void addDAO(Ioc ioc, String beanName, String mapperLocation, boolean one) {
        ...
        if (dataSource instanceof DataSource) {
            ...
            Resource[] array = StreamSupport.stream(Splitter.on(",").split(mapperLocation).spliterator(), false).map(it -> new ClassPathResource(it)).toArray(Resource[]::new);
            ...
            mappers.forEach(it -> {
                Object proxy = factory.getConfiguration().getMapperRegistry().getMapper(it, factory.openSession());
                if (one) {
