...
public class InetUtils {
    ...
    static {
        useOnlySiteLocalInterface = Boolean.parseBoolean(PropertyUtil.getProperty(Constants.USE_ONLY_SITE_INTERFACES));

        List<String> networks = PropertyUtil.getPropertyList(Constants.PREFERRED_NETWORKS);
        for (String preferred : networks) {
            preferredNetworks.add(preferred);
        }

        List<String> interfaces = PropertyUtil.getPropertyList(Constants.IGNORED_INTERFACES);
        for (String ignored : interfaces) {
            ignoredInterfaces.add(ignored);
        }

        String nacosIp = System.getProperty(Constants.NACOS_SERVER_IP);
        if (StringUtils.isBlank(nacosIp)) {
            nacosIp = PropertyUtil.getProperty(Constants.IP_ADDRESS);
        }

        if (StringUtils.isBlank(nacosIp)) {
            nacosIp = System.getenv(Constants.NACOS_SERVER_IP);
            log.info("get nacos server ip from env:{}", nacosIp);
        }
        ...
        if (StringUtils.isBlank(selfIp)) {
            preferHostnameOverIp = Boolean.getBoolean(Constants.SYSTEM_PREFER_HOSTNAME_OVER_IP);
