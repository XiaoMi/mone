...
public class Mvc {
    ...
    private void setConfig(Ioc ioc) {
        Boolean useCglib = Boolean.valueOf(ioc.getBean("$cglib", "true"));
        Boolean allowCross = Boolean.valueOf(ioc.getBean("$allow-cross-domain", "false"));
        Boolean responseOriginalValue = Boolean.valueOf(ioc.getBean("$response-original-value", "false"));
        Boolean download = Boolean.valueOf(ioc.getBean("$mvc-download", "false"));
        int poolSize = Integer.valueOf(ioc.getBean("$mvc-pool-size", "200"));
        this.mvcConfig.setAllowCross(allowCross);
        this.mvcConfig.setDownload(download);
        this.mvcConfig.setUseCglib(useCglib);
        this.mvcConfig.setResponseOriginalValue(responseOriginalValue);
        this.mvcConfig.setPoolSize(poolSize);
