...
public class ApplyUtil {
    ...
    public static List<ApplyVo> toVoList(List<ApplyEntity> entitys) {
        if ( CollectionUtils.isEmpty(entitys)) {
            return null;
        }
        List<ApplyVo> voList = new ArrayList<>(entitys.size());
        entitys.stream().forEach(e->voList.add(toVo(e)));
        return voList;
    }

    public static ApplyVo toVo(ApplyEntity entity) {
        if (entity == null) {
            return null;
        }
