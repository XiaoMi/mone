...
import io.opentelemetry.javaagent.instrumentation.jedis.v4_0.connect.JedisConnectionRequest;
...
import static io.opentelemetry.javaagent.instrumentation.jedis.v4_0.connect.JedisConnectSingletons.connectInstrumenter;
...
public class JedisConnectionInstrumentation implements TypeInstrumentation {
  ...
  public static class ConnectAdvice {
    ...
    public static void stopSpan(
            @Advice.FieldValue("socket") Socket socket,
            @Advice.Thrown Throwable throwable) {
      // 记录jedis connect异常trace
      if (throwable == null) {
        return;
      }
      Context parentContext = currentContext();
      JedisConnectionRequest request = JedisConnectionRequest.create();
      request.setSocket(socket);
