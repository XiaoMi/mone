...
public abstract class HttpServerTracer<REQUEST, RESPONSE, CONNECTION, STORAGE> extends BaseTracer {
  ...
  public void end(Context context, RESPONSE response, long timestamp) {
    ...
    setStatus(span, responseStatus(response), bussinessStatus(response), bussinessMessage(response));
  ...
  public void endExceptionally(
    ...
    Span span = Span.fromContext(context);
    if (response == null) {
      setStatus(span, 500, bussinessStatus(response), bussinessMessage(response));
    } else {
      setStatus(span, responseStatus(response), bussinessStatus(response), bussinessMessage(response));
    }
    end(context, timestamp);
  ...
  private static void setStatus(Span span, int status, String bussinessCode, String message) {
    span.setAttribute(SemanticAttributes.HTTP_STATUS_CODE, (long) status);
    StatusCode statusCode = HttpStatusConverter.statusFromHttpStatus(status, SERVER);
    if (statusCode != StatusCode.UNSET) {
      span.setStatus(statusCode);
      return;
    }
    // check bussinessCode
    if(!StringUtils.isNullOrEmpty(bussinessCode) && bussinessCode.startsWith("5")){
      span.setStatus(StatusCode.ERROR);
      span.recordException(new HttpServletException("bussiness code: "+bussinessCode+", message: "+message));
    }
  }
  ...
  protected abstract String bussinessStatus(RESPONSE response);
