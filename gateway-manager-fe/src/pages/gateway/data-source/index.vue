...
<template>
  <d2-container>
    ...
    <!-- 添加 & 更新 -->
    <el-dialog width="780px" :title="title" :visible.sync="formVisible" :close-on-click-modal=false>
      <el-form
        :model="form"
      ...
      >
        ...
        <template v-if="form.type == 3">
          ...
          <el-form-item label="redis 密码" prop="passWd">
            <el-input v-model="form.passWd" type="password"></el-input>
          </el-form-item>
        </template>
...
export default {
  ...
  watch: {
    'form.type': {
      handler () {
        if (this.form.type === 3) {
          this.rules.passWd = [{ required: false }]
        } else {
          this.rules.passWd = [{ required: true, message: '必填字段', trigger: 'blur' }]
        }
      }
    }
  }
