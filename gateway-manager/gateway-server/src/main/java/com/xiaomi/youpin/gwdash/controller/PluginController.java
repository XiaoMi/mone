...
//        Cnd cnd = Cnd.where("tenant", "=", tenantComponent.getTenement());
//        List<PluginInfoBo> pluginList = dao.query(PluginInfoBo.class, cnd);
...
//            @org.springframework.web.bind.annotation.RequestParam("commitId") String commitId,
//            @org.springframework.web.bind.annotation.RequestParam(value = "domain", defaultValue = "git.n.xiaomi.com", required = false) String domain) throws IOException {
//        SessionAccount account = loginService.getAccountFromSession(request);
//        if (!Optional.ofNullable(account).isPresent()) {
//            log.warn("[AccountController.newApiInfo] current user not have valid account info in session");
//            response.sendError(401, "未登录或者无权限");
...
//        }
//
//        return pluginService.createPlugin(account, url, projectId, group, name, commitId, domain);
//    }
//
//    /**
//     * 编辑plugin
...
//        }
//
//        return pluginService.editPlugin(account, pluginId, url, projectId, commitId, domain);
//    }
//
//
//    /**
...
//            @org.springframework.web.bind.annotation.RequestParam("group") String group,
//            @org.springframework.web.bind.annotation.RequestParam("name") String name,
//            @org.springframework.web.bind.annotation.RequestParam(value = "domain", defaultValue = "git.n.xiaomi.com", required = false) String domain
//    ) throws GitAPIException, IOException {
//        SessionAccount account = loginService.getAccountFromSession(request);
//        return pluginService.getGitlabCommits(domain, group, name, account);
//    }
//
//
//    /**
...
//                CompileRecord compileRecord = compilationService.getCompileRecord(it.getCompileId());
//                if (null == compileRecord) {
//                    return false;
//                }
//                if (compileRecord.getStep() == TaskStep.upload.ordinal()
//                        && compileRecord.getStatus() == TaskStatus.success.ordinal()) {
//                    it.setData(myKsyunService.getFileByDownloadKey(compileRecord.getJarKey()));
...
//            teslaGatewayServiceGroup.startPlugin(tenant, plugInfo);
//        } catch (Throwable ex) {
...
//    }
//
//    @RequestMapping(value = "/api/plugin/clear/authority", method = RequestMethod.POST, consumes = {"application/json"})
//    public Result<Boolean> clearAuthority(HttpServletRequest request, @RequestBody PluginStopParam param) {
//        SessionAccount account = loginService.getAccountFromSession(request);
//        if (BizUtils.isSuperRole(account)) {
//            PluginInfoBo info = dao.fetch(PluginInfoBo.class, param.getId());
//            return pluginService.clearAuthority(info);
//        }
//        return new Result<>(0, "无权限操作", false);
//    }
//
//
//    /**
...
//    public Result<List<GroupInfoBO>> groupList() {
//        List<Record> list = dao.query("gateway_server_info", null, null, "distinct `group`");
//        List<GroupInfoBO> data = list.stream().map(it -> {
//            GroupInfoBO groupInfo = new GroupInfoBO();
