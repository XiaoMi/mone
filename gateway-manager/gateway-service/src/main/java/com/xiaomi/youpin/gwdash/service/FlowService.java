...
import com.xiaomi.youpin.gwdash.dao.model.ApprovalType;
...
public class FlowService {
    ...
    public void agree(int id) {
        Approval approval = dao.fetch(Approval.class, Cnd.where("id", "=", id));
        if (null != approval) {
     ...
     * @return
     */
    public List<Approval> list(int status) {
        return dao.query(Approval.class,
                Cnd.where("status", "=", status));
    }

    public Map<String, Object> list(int status, int page, int pageSize) {
