...
final class GrpcSpanStatusExtractor implements SpanStatusExtractor<GrpcRequest, Status> {
  @Override
  public StatusCode extract(GrpcRequest request, Status status, @Nullable Throwable error, SpanKind spanKind) {
    if (status == null) {
    ...
    return SpanStatusExtractor.getDefault().extract(request, status, error, spanKind);
  }
}
