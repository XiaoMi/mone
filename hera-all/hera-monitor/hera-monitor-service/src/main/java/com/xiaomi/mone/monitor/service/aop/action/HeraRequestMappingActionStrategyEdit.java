...
@Slf4j
@Service
public class HeraRequestMappingActionStrategyEdit extends HeraRequestMappingActionArg2<HttpServletRequest, AlarmRuleRequest, Result>{
    ...
    public void afterAction(HttpServletRequest arg1, AlarmRuleRequest arg2, HeraReqInfo heraReqInfo, Result result) {
        Integer id = HeraRequestMappingContext.get("strategyId");
        if (id == null) {
            id = arg2.getStrategyId();
        }
        Pair<AlarmStrategy, List<AppAlarmRule>> pair = heraRequestMappingActionStrategyHelper.queryStrategyById(id);
