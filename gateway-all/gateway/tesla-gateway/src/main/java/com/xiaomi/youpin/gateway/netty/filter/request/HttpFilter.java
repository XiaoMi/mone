...
public class HttpFilter extends RequestFilter {
    ...
    public FullHttpResponse doFilter(FilterContext context, Invoker invoker, ApiInfo apiInfo, FullHttpRequest request) {
        ...
        if (routeType == RouteType.Http.type()) {
            ...
            try {
                if (sideCarFilterService.supportSideCar(SideCarEnum.http)) {
                    return sideCarFilterService.callHttpFilterSideCar(context, apiInfo, request);
                }
                log.debug("HttpFilter.doFilter, context: {}, apiinfo : {}", context, apiInfo);
                return client.call(context, apiInfo, request);
            } finally {
                long useTime = sw.elapsed(TimeUnit.MILLISECONDS);
                context.setRpcUseTime(useTime);
                HashMap<String, String> events = new HashMap<>(2);
                events.put("time", useTime + "ms");
                events.put("timeout", apiInfo.getTimeout() + "ms");
                context.addTraceEvent("call_http_event", events);
            }
        }
