...
public class RequestFilterChain {
    ...
    @DubboReference(check = false, group = "${dubbo.group}")
    ...
    public FullHttpResponse doFilter(ApiInfo apiInfo, FullHttpRequest request, RequestContext context) {
        ...
        try {
            ...
            // users want to know which env they are in their downstream applications
            ctx.getAttachments().put("envGroup", this.envGroup);
            ctx.getAttachments().put("initGroup", this.initGroup);
            ...
            ctx.setSpanId(TraceIdUtil.spanId());
            try {
                lock.readLock().lock();
