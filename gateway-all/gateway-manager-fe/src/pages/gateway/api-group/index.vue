  ...
  <d2-container>
    ...
    <d2-module>
      <el-table stripe :data="tableData" class="table-list">
        ...
        <el-table-column prop="baseUrl" label="Api前缀" style="min-width:260px"></el-table-column>
        <el-table-column label="跨域单元">
          <template slot-scope="scope">
            <b>{{scope.row.metaDataName ? `${scope.row.metaDataName}：` : ''}}</b>{{scope.row.domainList && scope.row.domainList.join(', ')}}
          </template>
        </el-table-column>
        <el-table-column v-if="userInfo.role === 1" label="操作" width="110" fixed="right">
          <template slot-scope="scope">
    ...
    <!--编辑-->
    <el-dialog :title="editFormTitle" :visible.sync="editFormVisible" width="800px" :close-on-click-modal=false>
      <el-form :model="formEdit" ref="formEdit" :rules="rules" label-width="110px" size="mini">
        ...
        <el-form-item label="名称" prop="name">
          <el-input v-model="formEdit.name" autocomplete="off" style="width:80%"></el-input>
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input type="textarea" v-model="formEdit.description" style="width:80%"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    addGroup (row) {
      const { name = '', description = '', baseUrl = '/mtop//' } = row
      const subBaseUrl = ''
      this.formEdit = {
        name,
        description,
      ...
      this.editFormTitle = '新增API Group'
      this.editFormVisible = true
    },
    // 编辑
    editRow (row) {
      const { id, name, description, baseUrl } = row
      let subBaseUrl = ''
      let preFix = '/mtop/'
      if (baseUrl) {
        subBaseUrl = baseUrl.substring(preFix.length)
        subBaseUrl = subBaseUrl.endsWith('/') ? subBaseUrl.substring(0, subBaseUrl.length - 1) : subBaseUrl.substring()
      }
      this.formEdit = {
        id,
        name,
      ...
      this.editFormTitle = '编辑API Group ' + id
      this.editFormVisible = true
    },
    handleRowEdit () {
      this.$refs['formEdit'].validate(valid => {
        ...
        this.formEdit.updatorId = this.userInfo.uuid
        const form = this.formEdit
        const baseUrl = form.subBaseUrl === '' ? '' : form.baseUrl
        const id = form.id
        service({
          url: id != null ? '/apigroup/update' : '/apigroup/new',
          method: 'post',
          data: {
            ...form,
            baseUrl
