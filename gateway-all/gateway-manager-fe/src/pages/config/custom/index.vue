  ...
  <d2-container>
    ...
    <d2-module>
      <el-table stripe :data='tableData' class='table-list'>
          ...
          <el-table-column label="key" prop="name" width="150"></el-table-column>
          <el-table-column label="配置内容" prop="content" width="280" show-overflow-tooltip></el-table-column>
          <el-table-column label="创建时间" prop="createTime" width="160"></el-table-column>
          <el-table-column label="更新时间" prop="updateTime" width="160"></el-table-column>
          <el-table-column label="操作" width="220" fixed="right">
            <template slot-scope="scope">
              <el-button type="text" size="mini" @click="showMenu(scope.row)">查看配置</el-button>
      ...
      <el-dialog
        ...
        <el-form ref='form' :model='form' :rules='rules' label-width='110px' size='mini'>
          <el-form-item label='key' prop='name'>
            <el-input :disabled="title == '编辑'" v-model="form.name"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    getInitList () {
      service({
        url: `/custom/config/list?page=${this.page}&pageSize=${this.pageSize}`,
        method: 'GET',
        data: {
          page: this.page,
    ...
    handleMenuConfig (tag, param) {
      if (param) {
        this.submitUrl = '/custom/config/update'
        this.form = { ...param }
      } else {
        this.submitUrl = '/custom/config/create'
        this.form = {
          name: '',
          content: ''
        }
    ...
    deleteMenuConfig (id) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        service({
          url: `/custom/config/delete?id=${id}`
        }).then(res => {
    ...
    showMenu (row) {
      this.codeMirrorMenuData = JSON.stringify(JSON.parse((row && row.content) || 'null'), null, 2)
