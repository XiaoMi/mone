...
public class PingFilter extends RequestFilter {
    ...
    public FullHttpResponse doFilter(FilterContext context, Invoker invoker, ApiInfo apiInfo, FullHttpRequest request) {
        if (apiInfo.getUrl().equals(URL)) {
            ...
            if ((sideCarFilterService.supportSideCar(SideCarEnum.ping))) {
                SideCarRequest req = SideCarRequest.newBuilder()
                        .setApp("pingSideCar")
                        .putAllAttachments(ImmutableMap.of("grpc","true"))
                        .setData(ByteString.copyFrom("ping".getBytes())).build();
                SideCarResponse res = sidecarService.call(req);
                ByteBuf buf = ByteBufUtils.createBuf(context, new String(res.getData().toByteArray()), configService.isAllowDirectBuf());
            ...
            String res = "pong:" + System.currentTimeMillis() + ":" + n + ":" + time + ":" + Thread.currentThread().getName();
