...
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) {
        if (userWs && TeslaConstants.WebSocketPath.equalsIgnoreCase(request.uri())) {
            ...
            initThreadPool(request, uri, apiInfo);
    ...
    private void initThreadPool(FullHttpRequest request, String uri, ApiInfo apiInfo) {
        logger.debug("call uri:{}", uri);
        if (apiInfo == null) {
            logger.info("HttpHandler.nullapiinfo, uri is: {}, request header is: {}", uri, request.headers());
        } else {
            String group = dispatcher.getGroup(apiInfo);
            if (StringUtils.isNotEmpty(group)) {
                dispatcher.getGroupMap().compute(group, (k, v) -> {
                    if (null == v) {
                        return dispatcher.createExecutorService(group, 200);
                    }
                    return v;
                });
            }
