...
public class DomainFilter extends RequestFilter {
    ...
    public FullHttpResponse doFilter(FilterContext context, Invoker invoker, ApiInfo apiInfo, FullHttpRequest request) {
        ...
        //check refer
        String referHeader = request.headers().get("Referer");
        Map<String, String> domainRefer = apiInfo.getDomainRefer();
        if (domainRefer == null || host == null) {
            log.info("DomainFilter.doFilter, domainRefer: {}, host: {}", domainRefer, host);
            return invoker.doInvoker(context, apiInfo, request);
        }
        String refer = domainRefer.get(host);
        log.debug("DomainFilter.doFilter, id:{}, host:{}, refer:{}, referHeader:{}, domainRefer:{}", apiInfo.getId(), host, refer, referHeader, domainRefer);
        if (StringUtils.isNotEmpty(refer)) {
            if (!checkHeader(refer, referHeader)) {
                return HttpResponseUtils.create(Result.fail(GeneralCodes.ParamError, "参数错误", "wrong header"));
            }
        }
        return invoker.doInvoker(context, apiInfo, request);
    }
    ...
    private boolean checkHeader(String param, String header) {
        if (!"".equals(param)) {
            ...
            if (Arrays.stream(paramArr).noneMatch(it -> host.endsWith(it))) {
                return false;
            }
        }
        return true;
