...
public class GatewayNamingService {
    ...
    public void init() {
        pool.scheduleAtFixedRate(() -> {
            try {
                log.info("GatewayNamingService map size:{} listenerMap size:{}, serviceIdMap size:{}", map.size(), listenerMap.size(), serviceIdMap.size());
                log.info("GatewayNamingService map keys:{} listenerMap keys:{}, serviceIdMap keys:{}", gson.toJson(map.keySet()), gson.toJson(listenerMap.keySet()), gson.toJson(serviceIdMap.keySet()));
            } catch (Throwable ex) {
                ...
                listenerMap.putIfAbsent(serviceName, event -> {
                    if (event instanceof NamingEvent) {
                        ...
                        if (value == null || value.isEmpty()) {
                            ...
                            log.info("GatewayNamingService, serviceName: {}, value: {}", serviceName, gson.toJson(value));
    ...
    public void unsubscribe(String serviceName, Long id) {
        try {
            ...
            log.info("unsubscribe: {}", serviceName);
